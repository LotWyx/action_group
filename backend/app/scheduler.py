...
async def check_upcoming_plan_dates() -> None:
    """Reminds employees in MAX about training-plan skills whose planned
    ...
    horizon = today + datetime.timedelta(days=settings.max_reminder_days_ahead)
    ...
    async with SessionLocal() as db:
        ...
        for item in items:
            ...
            if not user or not user.max_chat_id or not skill:
                continue
            ...
            sent = await max_notify.send_message(user.max_chat_id, text)
            if sent:
...
def start_scheduler() -> AsyncIOScheduler:
    ...
    scheduler.add_job(
        ...
        "interval",
        hours=settings.max_reminder_check_interval_hours,
        id="max_plan_reminders",
        next_run_time=datetime.datetime.now(),
    )
    scheduler.start()
    logger.info("MAX reminder scheduler started (every %sh)", settings.max_reminder_check_interval_hours)
    return scheduler
